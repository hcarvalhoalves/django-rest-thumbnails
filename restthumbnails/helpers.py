...
class InvalidThumbnailError(Exception):
    pass
...
def parse_size(size):
    ...
    if not match or not any(match.groups()):
        raise InvalidThumbnailError("%s is not a valid size string." % size)
    return map(lambda x: 0 if not x else int(x), match.groups())


def parse_method(method):
    if method not in ['crop', 'smart', 'scale']:
        raise InvalidThumbnailError("%s is not a valid method string." % method)
    return method


def to_hash(source, size, method):
    """
    Get a unique hash based on file path, size and SECRET_KEY.
    """
    secret_sauce = source + size + method + settings.SECRET_KEY
